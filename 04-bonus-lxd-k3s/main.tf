...
resource "null_resource" "k3s_server" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      set -e
      lxc exec ${self.triggers.instance_name} -- sh -c 'curl -sfL https://get.k3s.io | sh -'
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      lxc exec ${self.triggers.instance_name} -- sh -c \
        'curl -sfL https://get.k3s.io | K3S_URL=https://${self.triggers.server_ip}:6443 K3S_TOKEN=${self.triggers.token} sh -'
    EOT
  }
